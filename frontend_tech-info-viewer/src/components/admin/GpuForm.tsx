...
export function GpuForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("GPU agregada correctamente al catálogo.");
      ...
      toast.error("No se pudo guardar la GPU. Verifica tu sesión.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto border-border/50 shadow-xl">
      <CardHeader>
        <CardTitle>Agregar Nueva GPU</CardTitle>
        <CardDescription>Ingresa los detalles técnicos de la tarjeta gráfica.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input id="name" name="name" placeholder="Ej: RTX 4090" required />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="vram" name="vram" type="number" required />
            </div>
          ...
          <Button type="submit" className="w-full" disabled={isLoading}>
            {isLoading ? "Guardando..." : "Guardar GPU"}
          </Button>
        </form>
      </CardContent>
    </Card>
